# � Supabase + React - Complete Concepts Guide
...
3. [🗄️ Database Tables & Relationships](#database-tables)
4. [🔗 Foreign Keys](#foreign-keys)
5. [⚡ Database Triggers](#triggers)
6. [🛡️ Row Level Security (RLS)](#rls)
7. [🔄 Realtime Subscriptions](#realtime)
8. [🧩 SQL JOINs](#joins)
...
const { data, error } = await supabase.auth.signUp({
  ...
  options: {
    data: {
      name: name, // 👈 Custom user data
      account_type: accountType, // 👈 "rep" or "admin"
    },
  },
});
```

💡 **Important:** Data in `options.data` gets stored in `raw_user_meta_data` field!
...
- ✅ User signs in → `session` goes from `null` to `{user data}`
- ✅ User signs out → `session` goes from `{user data}` to `null`
- ✅ Page refresh → Session gets re-fetched
- ✅ Token refresh → Supabase auto-refreshes tokens

---

## 🗄️ Database Tables & Relationships {#database-tables}
...
│ id (PK)         │◄────────┤ id (PK, FK)      │
│ email           │         │ name             │
│ encrypted_pw    │         │ account_type     │
│ raw_user_meta_  │         │ created_at       │
│   data (JSON)   │         └──────────────────┘
└─────────────────┘                   ▲
                                      │ FK
                           ┌──────────┴──────────┐
...
**In our app:** `sales_deals.user_id → user_profiles.id`

```
┌─────────────┐         ┌──────────────┐
│sales_deals  │         │user_profiles │
├─────────────┤         ├──────────────┤
│user_id (FK) │────────→│id (PK)       │
│value        │         │name          │
...
1. **🔍 Check:** FK says _"Wait! Let me check `user_profiles` table..."_
2. **❓ Question:** Does `id = "abc123"` exist in `user_profiles`?
   - ✅ **YES** → _"Okay, insert the deal!"_
   - ❌ **NO** → _"ERROR! This user doesn't exist. Deal rejected!"_
...
// ✅ Trigger creates user in user_profiles

// Step 2: User tries to add deal
const newDeal = { user_id: currentUser.id, value: 1000 };
// ✅ FK checks: "Is this user in user_profiles?"
...
**Key Takeaway:** FK = Database Bouncer 🚪  
...
- No `user_id` check → can add deals for anyone!
...
// ✅ Policy 1 checks: Is user_id = current user? Is user a rep? → PASS!
...
// ✅ Policy 2 checks: Is user an admin? → PASS! (no user_id check needed)
```
...
⚠️ **Without this, policies won't work!**
...
```
User A adds a deal → Database updates
              ↓
...
2. ⚡ Trigger creates profile in user_profiles
   ↓
3. 🛡️ RLS policies check user's account_type
   ↓
4. ✅ Rep adds deal (only their own)
   ↓
5. 🔗 FK validates user_id exists
   ↓
6. 💾 Deal inserted into sales_deals
   ↓
7. 🔄 Realtime notifies all connected users
   ↓
8. 🧩 JOIN fetches names + sums for chart
   ↓
...
**🎓 Remember:** Supabase = PostgreSQL + Auth + Realtime + Security, all in one! �
